...
class DockerParams:
    ...
    def for_type_arch(cls, build_type, arch):
        prefix = {
            TYPE_DOCKER: "tom/webserver",
            TYPE_HA_ADDON: "tom/webserver-hassio",
            TYPE_LINT: "tom/webserver-lint",
        }[build_type]
...
def main():
    ...
    if args.command == "build":
        ...
        cache_img = f"code.skaro.sk/{params.build_to}:{cache_tag}"

        imgs = [f"{params.build_to}:{tag}" for tag in tags_to_push]
        imgs += [f"code.skaro.sk/{params.build_to}:{tag}" for tag in tags_to_push]
        ...
        targets += [f"code.skaro.sk/{manifest}:{tag}" for tag in tags_to_push]
        # 1. Create manifests
        for target in targets:
            cmd = ["docker", "manifest", "create", target]
            for arch in ARCHS:
                src = f"{DockerParams.for_type_arch(args.build_type, arch).build_to}:{args.tag}"
                if target.startswith("code.skaro.sk"):
                    src = f"code.skaro.sk/{src}"
                cmd.append(src)
            run_command(*cmd)
        # 2. Push manifests
        for target in targets:
